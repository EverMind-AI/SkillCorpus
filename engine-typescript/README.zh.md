...
3. **融合** —— 加权 Reciprocal Rank Fusion（K = 60）**按位次**合并各列表，因为本地 BM25 分数和目录质量分不是可比的数值。同名命中收敛为位次更优的那一份。
4. **补全** —— 仅带目录元数据进来的候选会被取回正文，每条通过融合的候选一次请求。
5. **Gate** —— 一次模型调用最多选出 `maxSelect` 条，并被要求宁可一条不选，也不要选不相关的。
6. **解析** —— 位于磁盘上的选中技能，其正文里的 `{baseDir}` 占位符和指向随附文件（`references/`、`scripts/` 等）的链接被改写为技能目录下的绝对路径，每处替换都做存在性检查。不改写的话，正文里的 `scripts/x.sh` 会被当作相对 agent cwd 的路径——那是错误的位置。
7. **注入** —— 选中结果渲染后追加到该 step 的消息中。
...
| `resolveRefs` | `true` | 把选中正文里的 `{baseDir}` 和随附文件链接改写为绝对路径。当 agent 与被检索的技能不共享文件系统时应关闭。 |
...
- **bundle 解压不引入依赖，靠手写的 ZIP 读取器。** Node 自带 `zlib`，但没有任何解析 ZIP 容器的东西，所以 `src/zip.ts` 自己读中央目录。它执行与 Python 实现相同的限制——路径包含检查、单文件 8 MiB、总量 64 MiB、后缀白名单——并且它的测试读的是 Python `zipfile` 写出的归档，而不是自己写的。宁可信任经审计的库而不是这里 130 行的部署，应当明确提出；这个接缝只有一个函数。
...
- **改写器的 `need_retrieval` 裁决有实测代价。** 对真实模型跑 6 轮，其中一条 query——"上周还能过的测试现在挂了，是哪个改动导致的"——在 `true`/`false` 之间约各占一半；判 `false` 就丢掉了一条排序和 gate 本都会选中的技能。prompt 里已写明"拿不准就检索"，模型并不总是照办。宁可多付一次扇出也不愿漏掉技能的部署，应当关掉改写（Python 侧 `rewrite: false`），只让 gate 做收窄。
