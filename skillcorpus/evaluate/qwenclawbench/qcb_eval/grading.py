...
# Optional KEY=VALUE file for judge credentials (env vars take priority).
# Override its location with QCB_ENV_FILE; default is <package_root>/.env
OPENCLAW_ENV_FILE = Path(os.environ.get(
    "QCB_ENV_FILE", str(Path(__file__).resolve().parent.parent / ".env")))

DEFAULT_JUDGE_MODEL = os.environ.get("JUDGE_MODEL", "anthropic/claude-opus-4.5")
    ...
    *,
    ...
    for attempt in range(JUDGE_API_MAX_RETRIES):
        ...
        try:
            ...
            last_exc = exc
            logger.warning("   LLM judge attempt %d/%d failed: %s",
                           attempt + 1, JUDGE_API_MAX_RETRIES, exc)

    # All direct-API attempts exhausted.
    raise RuntimeError(
        f"LLM judge failed after {JUDGE_API_MAX_RETRIES} attempts: {last_exc}")
