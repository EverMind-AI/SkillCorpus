/**
 * `skill_search`, the on-demand mode's whole surface (OpenClaw 1.x).
 *
...
import type { AgentTool, OpenClawPluginApi, ToolResult } from '../src/openclaw-types.ts'
...
async function skillsDir(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'openclaw-tool-'))
