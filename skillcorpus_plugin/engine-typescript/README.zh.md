...
1. **改写** —— 一次模型调用把消息清洗成检索查询，剔除路径、id 和样板文字。它无权判定该轮不需要技能：那个裁决实测下来接近抛硬币，而且是在看不到任何候选的情况下做的，因此归 gate——gate 能看到候选清单和该 agent 的工具清单。
...
| `hubEndpoint` | `https://skillhub.evermind.ai` | EverMind SkillHub，或任何实现同一 API 的自建服务；空值关闭。 |
