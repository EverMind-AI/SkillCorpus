"""源注册表 + 发现 + 全量 fetch — 读 sources.yaml, 发现并并发 clone 所有 repo.

单一真相源 = ``sources.yaml``。``fetch.py``(全量 crawl)和
``scripts/refresh_loop.py``(定时刷新)都从这里读, 不再各自硬编码。

每个源条目带 ``type``, 路由到对应的 discovery handler:

    git_clone       — repo 本身
    readme_scrape   — clone awesome 清单 repo + 扒 README 外链
    index_api       — REST API 分页 (params: api_url/items_key/url_field/has_next_key)
    json_catalog    — clone repo + 解析 JSON 目录 (params: repo/json_path/url_field)
    sitemap_scrape  — 抓 sitemap index + 子 sitemap → owner/repo (params: sitemap_url)
    lobehub_json    — repo 本身 (clone 后由 lobehub_to_skills.py 转换, 见 refresh_loop)

``discover_repos(entry)`` 统一返回 ``list[(owner, repo)]`` —— 调用方负责
clone + ingest。``lobehub_json`` 也返回 [(owner,repo)], 但 ingest 时需特殊转换。

本模块同时持有底层 clone 原语 (``clone_repo`` / ``extract_repos_from_readme``
/ ``GH_RE`` / ``FETCHED``), fetch.py 反向从这里 import (避免循环依赖)。
"""
...
# github URL 正则: 提 (owner, repo), 容忍 /tree/<branch>/<path> 后缀
GH_RE = re.compile(
...
# ---------------------------------------------------------------------------
# 底层 clone 原语 (从 fetch.py 下沉, 供 fetch.py + 本模块 discovery 复用)
# ---------------------------------------------------------------------------
def extract_repos_from_readme(readme_path: Path) -> list[tuple[str, str]]:
    """扫一个 README 里的所有 github.com/<owner>/<repo> 链接, 去重."""
...
def clone_repo(owner: str, repo: str, timeout: int = 180,
    ...
    """Clone github.com/<owner>/<repo> 到 FETCHED/. Returns (path, status),
    status ∈ {cloned, exists, fail}.

    GIT_TERMINAL_PROMPT=0 + GIT_ASKPASS=true: 私有/需认证 repo 立即失败而非卡超时.
    permanent 失败 (404/auth) 不重试; 瞬态失败重试一次.
    """
    import os as _os
...
# ---------------------------------------------------------------------------
# 共享提取器 (合并原 _skillsdirectory/_skillsmp/_skillmanager 里重复的 GH_RE 提取)
# ---------------------------------------------------------------------------
def _extract_github_repos(
    items: list[dict], url_field: str, fullname_field: str | None = None,
) -> list[tuple[str, str]]:
    """从一批 dict 里抽 distinct (owner, repo). 优先用 ``fullname_field``
    (``owner/repo`` 直字段), 否则从 ``url_field`` 的 github URL 正则提取."""
    seen: set[tuple[str, str]] = set()
    out: list[tuple[str, str]] = []
    for x in items:
        owner = repo = None
...
def _index_api_repos(
    ...
    """通用 REST API 分页器 (吸收 skillsdirectory + skillsmp).

    每页 GET ``{api_url}?{page_param}=N&{limit_param}={page_size}``, 从
    ``d[items_key]`` 抽 repo, 用 ``d['pagination'][has_next_key]`` 判停。
    """
...
def _json_catalog_repos(
    ...
    """clone ``repo``, 解析其 ``json_path`` JSON catalog, 抽 distinct repo
    (吸收 skillmanager)。支持 .gz fallback。"""
    ...
    gz = Path(str(jp) + ".gz")  # 同名 .gz fallback (某些 commit 只发 gzip 版)
    if jp.exists():
...
def _sitemap_repos(sitemap_url: str, limit: int = 0) -> list[tuple[str, str]]:
    """抓 sitemap (index 或叶), 提 skill 页 URL 的 /<owner>/<repo>/<skill> →
    distinct (owner, repo) (新写, 用于 skills.sh)。

    sitemap index 的 <loc> 指向子 sitemap (.xml); 叶 sitemap 的 <loc> 指向
    skill 页。递归一层处理 index。
    """
    seen: set[tuple[str, str]] = set()
    out: list[tuple[str, str]] = []
    ...
    for u in page_urls:
        # https://host/<owner>/<repo>/<skill>  → 取 path 前两段
        m = re.match(r"https?://[^/]+/([^/]+)/([^/]+)/[^/]+", u)
        if not m:
            continue
        owner, repo = m.group(1), m.group(2).rstrip(".")
        if owner in ("skills", "owners", "about", "search"):  # 非 owner 的站内路径
            continue
        key = (owner, repo)
        if key in seen:
            continue
...
def load_registry(yaml_path: Path | str = DEFAULT_YAML) -> list[dict]:
    """读 sources.yaml, 返回 entries 列表 (每条含 name/repo/type/...)。"""
...
def discover_repos(entry: dict, timeout: int = 180, limit: int = 0) -> list[tuple[str, str]]:
    """按 entry['type'] 返回该源要 clone 的 distinct (owner, repo) 列表.

    git_clone / lobehub_json → 自身; 其余 → 对应 discovery。
    """
    t = entry["type"]
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="只发现并列出要 clone 的 repo, 不实际 clone "
                         "(仍会扒 awesome 清单 / 打 API / 抓 sitemap 以得到完整列表)")
    ap.add_argument("--max-from-readme", type=int, default=0,
                    help="每个 readme_scrape 源最多取多少外链 (0 = 不限, 调试用)")
    ap.add_argument("--timeout", type=int, default=180,
                    help="单个 repo clone 超时秒数")
    ap.add_argument("--workers", type=int, default=16,
                    help="并发 clone 线程数 (网络 I/O bound)")
    ap.add_argument("--config", type=Path, default=None,
                    help="sources.yaml 路径 (默认 skill_library/sources.yaml)")
    args = ap.parse_args()

    FETCHED.mkdir(parents=True, exist_ok=True)
    registry = load_registry(args.config) if args.config else load_registry()

    # ---- Step 1: 遍历注册表, 逐源 discover ----
    all_repos: dict[tuple[str, str], str] = {}  # (owner, repo) → source tag
    print("=" * 70)
    print(f"[Step 1] 发现阶段 — 注册表 {len(registry)} 个源")
    print("=" * 70)
    for entry in registry:
        ...
        try:
            ...
            print(f"  ✗ [{typ}] {name}: discover 失败 {e!r}")
            continue
        ...
        for o, r in repos:
            if (o, r) not in all_repos:
                ...
                new += 1
        print(f"  [{typ}] {name}: 发现 {len(repos)} repo (+{new} 新)")

    # ---- Step 2: 统一 clone ----
    print()
    print("=" * 70)
    print(f"[Step 2] 合并去重后共 {len(all_repos)} 个 repo")
    print("=" * 70)

    if args.dry_run:
        for (owner, repo), tag in sorted(all_repos.items()):
            print(f"  [{tag}] {owner}/{repo}")
        print(f"\n(dry-run, 不实际 clone) 共 {len(all_repos)} 个 repo")
        return
